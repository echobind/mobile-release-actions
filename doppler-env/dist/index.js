...
const main = async () => {
    ...
    await (0, child_process_1.exec)('(curl -Ls --tlsv1.2 --proto "=https" --retry 3 https://cli.doppler.com/install.sh || wget -t 3 -qO- https://cli.doppler.com/install.sh) | sudo sh', (err) => {
        if (err) {
    ...
    await (0, child_process_1.exec)(`doppler secrets download --no-file --format=env --token=${token} > .env`, (err) => {
        if (err) {
            core.setFailed(err instanceof Error ? err.message : JSON.stringify(err));
        }
        else {
