...
const main = async () => {
    ...
    (0, child_process_1.exec)('(curl -Ls --tlsv1.2 --proto "=https" --retry 3 https://cli.doppler.com/install.sh || wget -t 3 -qO- https://cli.doppler.com/install.sh) | sudo sh', (err) => {
        if (err) {
            core.setFailed(err instanceof Error ? err.message : JSON.stringify(err));
        }
        else {
            core.info('Doppler CLI installed');
            (0, child_process_1.exec)(`doppler secrets download --no-file --format=json --token=${token} > temp.json`, async (err) => {
                if (err) {
                    core.setFailed(err instanceof Error ? err.message : JSON.stringify(err));
                }
                else {
                    core.info('Secrets downloaded');
                    const secretsFile = await promises_1.default.readFile(`${process.env.GITHUB_WORKSPACE}/temp.json`, 'utf8');
                    const easJsonFile = await promises_1.default.readFile(`${process.env.GITHUB_WORKSPACE}/eas.json`, 'utf8');
                    ...
                    await promises_1.default.writeFile(`${process.env.GITHUB_WORKSPACE}/eas.json`, JSON.stringify(updatedEasJson, null, 2));
